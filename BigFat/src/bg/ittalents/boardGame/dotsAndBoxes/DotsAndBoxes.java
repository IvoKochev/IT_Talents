...
public class DotsAndBoxes extends Game {
	...
	private boolean isPlayerWin() {
		if (isGameOver()) {
			...
			return countForA > countForO ? true : false;
		}
		return false;
	...
	private boolean isBotWin() {
		if (isGameOver()) {
			...
			return countForO > countForA ? true : false;
		}
		return false;
	}
	...
	private boolean isFilled(int row, int col, int row1, int col1) {
		if (row > row1) {
			if (this.getBoard().getMatrixx()[row - 1][col].equals(" ")) {
				return true;
			} else {
				return false;
			}
		...
		if (row1 > row) {
			if (this.getBoard().getMatrixx()[row1 - 1][col].equals(" ")) {
				return true;
			} else {
				return false;
			}
		...
		if (col > col1) {
			if (this.getBoard().getMatrixx()[row][col - 1].equals(" ")) {
				return true;
			} else {
				return false;
			}
		...
		if (col1 > col) {
			if (this.getBoard().getMatrixx()[row][col1 - 1].equals(" ")) {
				return true;
			} else {
				return false;
			}
	...
	private boolean isInBounds(int row, int col, int row1, int col1) {
		if (row >= 0 && row < this.getBoard().getRowLength() / 2 && row1 >= 0
